...
export const AutoField = {
	...
	monthlyJoural(dv) {
		...
		const pages = dv
			...
			.where((page) => {
				...
				if (
					atShort === currentAtShort &&
					fm.getProject() === "monthly"
				) {
					return true;
				}
	...
	log(dv) {
		...
		for (let i = 0; i < pages.length; i++) {
			const page = pages[i];
			Assert.True(page !== undefined, "ass ert failed");
			const fm = new FrontmatterJS(page);
...
export class Generator {
	...
	daily() {
		/*
		type: 3
		uuid: "32110f57-c917-4076-a978-66a745ae5457"
		created_at: "2025-10-10T10:32:00+02:00"
		version: "0.0.6"
		status: "todo"
		priority: 0
		at: "2025-10-19T10:32:00+02:00"
		tags: 
		  - project/daily
	  */
		const dt = new Date();
	...
	projectTasksSheetRelation(dv) {
		...
		for (const page of pages) {
			...
			if (this.noteHelper.isDoable(page)) {
				...
				if (
					fm.fm.needs !== undefined &&
					this.noteHelper.hasPendingDependencies(fm.fm.needs)
				) {
					...
					if (
						fm.fm.at !== undefined &&
						fmAt.getTime() > currentAt.getTime()
					) {
						page.cause = "at";
					} else if (
						fm.fm.after !== undefined &&
						fmAfter.getTime() < currentAt.getTime()
					) {
						page.cause = "after";
					} else {
						page.cause = "unknown";
