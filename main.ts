...
const NAMED_LINK_TYPES = ["assets", "source", "push", "pull"] as const;

type NamedLinkType = (typeof NAMED_LINK_TYPES)[number];
...
class NamedLinkSuggestModal extends FuzzySuggestModal<NamedLinkType> {
  constructor(
    app: App,
    private editor: Editor,
    private link: WikiLinkMatch,
  ) {
    super(app);
  ...
  getItems(): NamedLinkType[] {
    return [...NAMED_LINK_TYPES];
  }

  getItemText(item: NamedLinkType): string {
    return item;
  }

  onChooseItem(item: NamedLinkType): void {
    ...
    if (this.editor.getRange(from, to) !== this.link.text) {
      new Notice("The wikilink changed before it could be named");
      return;
    }
...
export default class MyPlugin extends Plugin {
  ...
  setManagedHeadingAlias(path: string, alias: string | undefined): void {
  ...
  async reconcileAllHeadingAliases(): Promise<void> {
    ...
    for (const path of Object.keys(this.settings.managedHeadingAliases)) {
      ...
      const matches =
        name === undefined ? [] : (filesByName.get(name) ?? []);
      if (
        matches.length === 1 &&
  ...
  openPdfExternally(file: TFile, subpath: string): void {
    if (!Helper.isUUID(file.parent.name)) {
      new Notice(`Invalid asset location: ${file.path}`);
      return;
    }
    ...
    const reportError = () => {
      if (!errorReported) {
        errorReported = true;
        new Notice(`Unable to open PDF: ${file.name}`);
      }
    };
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      editorCallback: (editor: Editor) => {
        ...
        if (link === null) {
          new Notice("No wikilink found on the current line");
          return;
        }
